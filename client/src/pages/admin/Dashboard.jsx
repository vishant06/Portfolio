...
const Dashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const homeImageData = await request('/assets/home-image').catch(() => null);
      setProjects(projectData);
      setMessages(messageData);
      setResume(resumeData);
      setHomeImage(homeImageData);
    } catch (error) {
  ...
  const uploadHomeImage = async (event) => {
    const file = event.target.files[0];
    if (!file) return;
    const payload = new FormData();
    payload.append('homeImage', file);
    try {
      const data = await request('/assets/home-image', { method: 'POST', body: payload });
      setHomeImage(data);
      setStatus('Home image updated.');
      event.target.value = '';
    } catch (error) {
      setStatus(error.message);
    }
        ...
        <div className="admin-grid">
          ...
          <div className="panel">
            ...
            <div className="home-image-preview">
              <img src={homeImage ? absoluteAsset(homeImage.fileUrl) : '/SAVE_20260619_172554.jpg'} alt="Current home profile" />
            </div>
            <label className="upload-box compact">
              <Upload size={22} />
              <span>Change portfolio home image</span>
              <input type="file" accept="image/*" onChange={uploadHomeImage} />
            </label>
          </div>
        </div>
